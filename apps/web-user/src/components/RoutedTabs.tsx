...
function Tab ({ value, children, routeRoot }: {
  value: string;
  ...
  return (
    <Tabs.Trigger value={value} asChild>
      <ChakraLink asChild>
        <StyledLink href={`${routeRoot}${value}`}>
          {children}
        </StyledLink>
      </ChakraLink>
    </Tabs.Trigger>
...
export function RoutedTabs ({
  ...
  const { query: { tab: tabKey }, isReady } = useRouter()

  if (!isReady) return null

  return (
    <Tabs.Root
      value={tabKey ? tabKey as string : tabs[0]?.key}
    >
      <Tabs.List>
        <For each={tabs}>
          {({ key: tKey, text: tTxt }) => <Tab routeRoot={routeRoot} key={tKey} value={tKey}>{tTxt}</Tab>}
        </For>
      </Tabs.List>
      <For each={tabs}>
        {({ key: tKey, content }) => <Tabs.Content key={tKey} value={tKey}>{content}</Tabs.Content>}
      </For>
    </Tabs.Root>
  )
}
